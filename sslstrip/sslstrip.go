...
var ignoredResponseHeaders = map[string]struct{}{
	// content length will change after sslstrip and needs to be recalculated
...
func normalizeUrl(link string) (string, error) {
	...
	newLink, _ := strings.CutPrefix(link, "//")

	originalUrl, err := url.Parse(newLink)
	fmt.Println("Ambyarr originalURL:", originalUrl)
...
func (s server) ServeHTTP(responseWriter http.ResponseWriter, req *http.Request) {
	...
	fmt.Println("Ambyarr URL:", req.URL.String())
	fmt.Println("Ambyarr Host:", req.Host)
	...
	// decompress body before sslstrip if compressed
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when sslstrip response: %q\n", err)
		return
	}
